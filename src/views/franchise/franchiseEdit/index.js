...
import FranchiseForm from "../franchiseEditForm";
...
import { getFranchise, edit_franchise } from "./store/dataSlice";
import isEmpty from "lodash/isEmpty";

injectReducer("FranchiseListEdit", reducer);

const WolooEdit = () => {
  const dispatch = useDispatch();
  const location = useLocation();
  const navigate = useNavigate();
  const wolooData = useSelector((state) => state.FranchiseListEdit.data.woloosList);
  const Id = location.pathname.substring(
    location.pathname.lastIndexOf("/") + 1
  );

  const loading = useSelector((state) => state.FranchiseListEdit.data.loading);
  ...
  const handleFormSubmit = async (values, setSubmitting) => {
    ...
    try {
      const success = await edit_franchise(formData);
      if (success) {
        setSubmitting(true);
  ...
  const popNotification = (keyword) => {
    toast.push(
      <Notification
        ...
        duration={2500}
      >
        Franchise successfully {keyword}
      </Notification>,
      {
        placement: "top-center",
      }
    ...
    <>
      <AdaptableCard className="h-full" bodyClass="h-full">
        <div className="flex items-center mb-4">
          <FaHandshake size={"30"} />
          <h3 className="ml-5">Franchise Edit</h3>
        </div>
