...
const UserOfferBulkDeleteConfirmation = () => {
  const dispatch = useDispatch();
  const dialogOpen = useSelector(
    (state) => state.userOfferList.data.bulkdeleteConfirmation
  );
  const selectedRows = useSelector(
    (state) => state.userOfferList.state.selectedRows
  );
  const tableData = useSelector((state) => state.userOfferList.data.tableData);
  let selectedUserOffer = [];
  for (let row of selectedRows) {
    if (row.isSelected == 1) {
      selectedUserOffer.push(row.id);
    }
  }
...
export default UserOfferBulkDeleteConfirmation;
