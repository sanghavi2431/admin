...
export const fetch_All__Categories = createAsyncThunk("fetch_All__Categories/Addactive", async (data) => {
  const response = await fetchAllCategories(data);
  return response.data.results.data;
});
export const fetch_All__SubCategories = createAsyncThunk("fetch_All__SubCategories/Addactive", async (data) => {
  const response = await fetchAllSubCategories(data);
  return response.data.results.data;
});
const dataSlice = createSlice({
  name: "blog_Sub_Category/activeAddform",
  initialState: {
    loading: false,
    categoryList:[],
    subCategoryData:[]
