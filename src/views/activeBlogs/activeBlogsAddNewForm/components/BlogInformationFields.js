...
const BlogInformationFields = (props) => {
  const { values, touched, errors } = props;
  const [selectedCategories, setSelectedCategories] = useState([])
  const dispatch = useDispatch();
  const categoryList = useSelector((state) => state.blogAddForm?.data?.categoryList);
  ...
  const subCategoryData = useSelector((state) => state.blogAddForm?.data?.subCategoryData);
...
export default BlogInformationFields;
