...
import { setAddSupervisorData } from "../store/dataSlice";
...
const AddSupervisorForm = ({ goToNextStep, goToPreviousStep }) => {
  const dispatch = useDispatch();
  const addSupervisorData = useSelector((state) => state?.clientOnboard?.data?.addSupervisorData);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      payload.append("user_id", addSupervisorData?.user_id);

      const response = await addSupervisor(payload);
      const user = response?.data?.results?.data;
      dispatch(setAddSupervisorData({ ...values, user_id: user?.value }));
      goToNextStep();
    } catch (error) {
      ...
      <div className="p-8 w-full">
        <div className="flex justify-center items-center gap-4 mb-10">
          ...
          <h2 className="text-2xl font-bold text-black text-center">
            Assign <span className="text-primary">Supervisor</span>
          </h2>
        </div>

        <Formik
          initialValues={addSupervisorData}
          ...
          {({ touched, errors, isSubmitting, values, setFieldValue }) => (
            <Form>
              <FormContainer className="flex flex-col gap-6">
                <FormItem label="Full Name*" invalid={errors.full_name && touched.full_name} errorMessage={errors.full_name}>
                  <Field type="text" name="full_name" component={Input} placeholder="Full Name *" />
                </FormItem>

                <FormItem label="Number*" invalid={errors.number && touched.number} errorMessage={errors.number}>
                  <Field type="text" name="number" component={Input} placeholder="Number *" />
                </FormItem>
