...
import { getBlock } from "../store/dataSlice";

const BlockDeleteConfirmation = () => {
  const dispatch = useDispatch();
  const dialogOpen = useSelector(
    (state) => state.blockList.state.deleteConfirmation
  );
  const selectedBlock = useSelector(
    (state) => state.blockList.state.selectedBlock
  );
  const tableData = useSelector((state) => state.blockList.data.tableData);
