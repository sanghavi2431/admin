...
import { downloadTemplateMappingUploadTemplate } from "@/services/taskManagement/templateMapService";
import { setclient, setlocation, setblock, getLocation, get_Blocks } from "../template_mappingUpload/store/dataSlice";
...
const TemplateMappingUploadFileForm = forwardRef((props, ref) => {
  ...
  // Redux state
  const locations = useSelector((state) => state.Template_mapping_Upload.data.locations);
  const blocks = useSelector((state) => state.Template_mapping_Upload.data.blocks);
  const loading = useSelector((state) => state.Template_mapping_Upload.data.loading);
  ...
  const downloadTemplate = async (values) => {
    if (!values.client_name || !values.client_name.value) {
      console.error("No building selected");
      return;
    }
    try {
      const response = await downloadTemplateMappingUploadTemplate(
        {
          client_id: values.client_name.value,
          block_id: values.block_name.value
        }
      );
