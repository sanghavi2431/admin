...
import { getdisplay_Addons, setCardsDetails, setAddonOption, getpurchasedPlanIdby_ClientId } from "./store/dataSlice";
...
export default function UpgradePlan() {
    ...
    const addOnOptions = useSelector((state) => state.UpgradePlan.data.addOnOptions)
    ...
    const purchasedPlanId = useSelector((state) => state.UpgradePlan.data.purchasedPlan)
    ...
    useEffect(() => {
        setVisibleoption(addOnOptions.filter((day) => {
            if (day?.flag) { return day }
        }))

    }, [addOnOptions])

    function addOnHandler(selectedItem) {
        dispatch(setAddonOption(addOnOptions?.map((day) => {
    ...
    const paymentHandler = async (finalAmount) => {
        ...
        try {
            ...
            if (success) {
                const options = {
                    key: process?.env?.RAZORPAY_API_KEY,                    amount: finalAmount * 100,
                    currency: "INR",
                    name: "Woloo",
    ...
    return (
        <>
            ...
            <div>
                <BsChevronLeft className="cursor-pointer" size={"24"} onClick={() => navigate("/subscriptionPlans")} />
                ...
                <div className="w-full h-full flex flex-col justify-center items-center text-black ">
                    ...
                    <div className="w-full flex flex-col gap-4 text-base border-2 p-4  mb-4 ">
                        ...
                        {isLogin && <div className="flex justify-between ">
                            ...
                            <div className="flex">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button type="button" className="border-2 w-6 bg-slate-100 rounded" onClick={() => { setQty(qty ? qty - 1 : qty); }}> - </button>
                                    <p>{defaultQty + qty}</p>
                                    <button type="button" className="border-2 w-6 bg-slate-100 rounded" onClick={() => { setQty(qty + 1); }}   > + </button>
                                </div>
                                <p className="w-20 flex items-end justify-end">₹ {(qty * AddOnAmt)?.toFixed(2)}</p>

                            </div>
                        </div>}
                        <div className="flex gap-2 items-center">
                            <p>Add on</p>
                            <Select value="" size="xs" options={visibleOption} className="w-40" onChange={(val) => { addOnHandler(val) }} />
                        </div>
                        {
                            addOnOptions.map((day) => {
                                if (!day?.flag) {
                                    return (
                                        <div className="flex justify-between bg-[#F9F9F9] p-2">
                                            ...
                                            <div>
                                                <div className="flex gap-2">
                                                    <div className="flex gap-2">
                                                        <button type="button" className="border-2 w-6 bg-slate-100 rounded" onClick={() => { decreaseQty(day) }}> - </button>
                                                        <p>{day.qty}</p>
                                                        <button type="button" className="border-2 w-6 bg-slate-100 rounded" onClick={() => { increaseQty(day); }}   > + </button>
                                                    </div>
                                                    <p className="w-20 flex items-end justify-end">₹ {(day.qty * day.addOnAmt)?.toFixed(2)}</p>
                                                    <button type="button" onClick={() => { deleteOnHandler(day) }}><MdDelete /></button>
                                                </div>
                        ...
                        <div className="flex justify-between text-lg font-semibold">
                            <p >Total</p>
                            <p>₹{(((+qty * +AddOnAmt) + +totalAmt + apiAddOnAmt) * 1.18).toFixed(2)}</p>
                        </div>
                    ...
                    <div className="w-full flex items-end justify-end gap-2">
                        <Button
                            ...
                            color="black"
                            onClick={() => {
                                paymentHandler((((+qty * +AddOnAmt) + +totalAmt + apiAddOnAmt) * 1.18).toFixed(2))
                            }}
