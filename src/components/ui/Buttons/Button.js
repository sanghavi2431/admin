...
const Button =  React.forwardRef((props, ref) => {
	const { children, size, color, shape, variant, block, icon, className, disabled, loading, active, danger,visibility, ...rest } = props
	const { themeColor, controlSize, primaryColorLevel } = useConfig()
	const formControlSize = useForm()?.size
	const inputGroupSize = useInputGroup()?.size
	const defaultClass = 'button shadow-custom'
	...
	const btnColor = () => {
		switch (variant) {
			case 'solid':
				return color ? solidColor() : 'bg-[#00C3DE] hover:bg-[#00c4debd]'
			case 'twoTone':
				return twoToneColor()
			case 'plain':
				return plainColor()
